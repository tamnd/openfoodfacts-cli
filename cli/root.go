// Package cli assembles the openfoodfacts command tree from the openfoodfacts
...
// NewApp assembles the kit application from the openfoodfacts domain. The
...
//
// To add a command, declare it in openfoodfacts/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openfoodfacts.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openfoodfacts.Domain{}).Register(app)
