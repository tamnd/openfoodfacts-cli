...
describes what users get. Pick a format with `-o`, or let openfoodfacts choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openfoodfacts <command> -o table   # aligned columns for reading
openfoodfacts <command> -o jsonl   # one JSON object per line, for piping
openfoodfacts <command> -o json    # a single JSON array
openfoodfacts <command> -o csv     # spreadsheet friendly
openfoodfacts <command> -o tsv     # tab-separated
openfoodfacts <command> -o url     # just the URL column
openfoodfacts <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openfoodfacts <command> --fields id,title,url
```
...
```bash
openfoodfacts <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openfoodfacts <command>            # a table, because this is a terminal
openfoodfacts <command> | wc -l    # JSONL, because this is a pipe
```
